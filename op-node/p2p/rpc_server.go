...
func (s *APIBackend) Self(ctx context.Context) (*PeerInfo, error) {
	recordDur := metrics.RecordRPCServerRequest("opp2p_self")
	defer recordDur()
	h := s.node.Host()
	nw := h.Network()
	pstore := h.Peerstore()
...
func (s *APIBackend) Peers(ctx context.Context, connected bool) (*PeerDump, error) {
	recordDur := metrics.RecordRPCServerRequest("opp2p_peers")
	defer recordDur()
	h := s.node.Host()
	nw := h.Network()
	pstore := h.Peerstore()
...
func (s *APIBackend) PeerStats(_ context.Context) (*PeerStats, error) {
	recordDur := metrics.RecordRPCServerRequest("opp2p_peerStats")
	defer recordDur()
	h := s.node.Host()
	nw := h.Network()
	pstore := h.Peerstore()
...
func (s *APIBackend) DiscoveryTable(_ context.Context) ([]*enode.Node, error) {
	recordDur := metrics.RecordRPCServerRequest("opp2p_discoveryTable")
	defer recordDur()
	if dv5 := s.node.Dv5Udp(); dv5 != nil {
...
func (s *APIBackend) BlockPeer(_ context.Context, p peer.ID) error {
	recordDur := metrics.RecordRPCServerRequest("opp2p_blockPeer")
	defer recordDur()
	if gater := s.node.ConnectionGater(); gater == nil {
...
func (s *APIBackend) UnblockPeer(_ context.Context, p peer.ID) error {
	recordDur := metrics.RecordRPCServerRequest("opp2p_unblockPeer")
	defer recordDur()
	if gater := s.node.ConnectionGater(); gater == nil {
		return NoConnectionGater
	} else {
...
func (s *APIBackend) ListBlockedPeers(_ context.Context) ([]peer.ID, error) {
	recordDur := metrics.RecordRPCServerRequest("opp2p_listBlockedPeers")
	defer recordDur()
	if gater := s.node.ConnectionGater(); gater == nil {
...
func (s *APIBackend) BlockAddr(_ context.Context, ip net.IP) error {
	recordDur := metrics.RecordRPCServerRequest("opp2p_blockAddr")
	defer recordDur()
	if gater := s.node.ConnectionGater(); gater == nil {
		return NoConnectionGater
	} else {
...
func (s *APIBackend) UnblockAddr(_ context.Context, ip net.IP) error {
	recordDur := metrics.RecordRPCServerRequest("opp2p_unblockAddr")
	defer recordDur()
	if gater := s.node.ConnectionGater(); gater == nil {
		return NoConnectionGater
	} else {
...
func (s *APIBackend) ListBlockedAddrs(_ context.Context) ([]net.IP, error) {
	recordDur := metrics.RecordRPCServerRequest("opp2p_listBlockedAddrs")
	defer recordDur()
	if gater := s.node.ConnectionGater(); gater == nil {
		return nil, NoConnectionGater
	} else {
...
func (s *APIBackend) BlockSubnet(_ context.Context, ipnet *net.IPNet) error {
	recordDur := metrics.RecordRPCServerRequest("opp2p_blockSubnet")
	defer recordDur()
	if gater := s.node.ConnectionGater(); gater == nil {
		return NoConnectionGater
	} else {
...
func (s *APIBackend) UnblockSubnet(_ context.Context, ipnet *net.IPNet) error {
	recordDur := metrics.RecordRPCServerRequest("opp2p_unblockSubnet")
	defer recordDur()
	if gater := s.node.ConnectionGater(); gater == nil {
		return NoConnectionGater
	} else {
...
func (s *APIBackend) ListBlockedSubnets(_ context.Context) ([]*net.IPNet, error) {
	recordDur := metrics.RecordRPCServerRequest("opp2p_listBlockedSubnets")
	defer recordDur()
	if gater := s.node.ConnectionGater(); gater == nil {
		return nil, NoConnectionGater
	} else {
...
func (s *APIBackend) ProtectPeer(_ context.Context, p peer.ID) error {
	recordDur := metrics.RecordRPCServerRequest("opp2p_protectPeer")
	defer recordDur()
...
func (s *APIBackend) UnprotectPeer(_ context.Context, p peer.ID) error {
	recordDur := metrics.RecordRPCServerRequest("opp2p_unprotectPeer")
	defer recordDur()
	if manager := s.node.ConnectionManager(); manager == nil {
		return NoConnectionManager
	} else {
...
func (s *APIBackend) ConnectPeer(ctx context.Context, addr string) error {
	recordDur := metrics.RecordRPCServerRequest("opp2p_connectPeer")
	defer recordDur()
	h := s.node.Host()
...
func (s *APIBackend) DisconnectPeer(_ context.Context, id peer.ID) error {
	recordDur := metrics.RecordRPCServerRequest("opp2p_disconnectPeer")
	defer recordDur()
